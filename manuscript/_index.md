# TIDEMARK — chapter index

| # | Title | Status |
|---|-------|--------|
| 00 | Front matter | done |
| 01 | The Wrong Tense | done |
| 02 | Be Boring | done |
| 03 | Tier 22 | done |
| 04 | The Ones Who Notice | done |
| 05 | Under the Line | done |
| 06 | The Kitchen at the Bottom of the World | done |
| 07 | The Wall of People Who Haven't Died Yet | done |
| 08 | The Boat With the Hole In It | done |
| 09 | What It Costs to Keep Bailing | done |
| 10 | Nothing Added Back | done |
| 11 | Where Nobody's Counting | done |
| 12 | Far Enough | done |
| 13 | Where It Stops Reaching | done |
| 14 | The Smooth Place | done |
| 15 | Something to Stand On | done |
| 16 | He Carried It Down Himself | done |
| 17 | The First Inch | done |
| 18 | The Shape of Days | done |
| 19 | Send Word | done |
| 20 | Something for the Bag | done |
| 21 | The Foot of It | done |
| 22 | The Cold Part | done |
| 23 | Back Under the Weather | done |
| 24 | Where People Had Been | done |
| 25 | A Man Passing Through | done |
| 26 | A Face Nobody Knew | done |
| 27 | The Next Lamp | done |
| 28 | The Parcel-Man | done |
| 29 | The Light One | done |
| 30 | Off My Back | done |
| 31 | Which Way the Far Men Come | done |
| 32 | The Long Run | done |
| 33 | Between Lamps | done |
| 34 | The First Fork | done |
| — | Ch28–34 weekly audit | done (2026-07-15) |
| 35 | The Way Down | done |
| 36 | Somebody's Line | done |
| 37 | The First Face Down There | done |
| 38 | What the Quiet Bought | done |
| 39 | More Than One Fire | done |
| 40 | The Edge of the Yards | done |
| 41 | The Talk of the Man | done |
| — | Ch35–41 weekly audit | done (2026-07-23) |
| 42 | The Fire Nobody Owned | done |
| 43 | The Middle of the Yards | done |
| 44 | The Cleared Shelf | done |
| 45 | The Light Up the Wet Way | done |
| 46 | What Came Up the Wet Way | done |
| 47 | Across the Light | done |
| 48 | What in Hell | done |
| — | Ch42–48 weekly audit | done (2026-07-31) |
| 49 | In the Light | done |
| 50 | Dated Out | done |
| 51 | More Than Telling | done |
| 52 | Two Dark Wrists | done |
| 53 | The Number I Put Down | done |
| 54 | The One Shelf | done |
| 55 | (next chapter) | pending (next run) |

_Weekly audits run as separate runs and do NOT take a chapter number — chapter numbering is continuous, no gaps. Audit windows so far: Ch7–13, Ch14–20, Ch21–27, Ch28–34, Ch35–41, Ch42–48 (every 7). The Ch42–48 audit is DONE (2026-07-31; all in notes/audit.md). Ch54 (the cost of the gate: Adz brings the night's failure into the morning light — his number is only whole in his file up in the count, so fetching it and surfacing into view are the same act; Maning doesn't take the ask back and doesn't decide) is DONE. THE NEXT RUN WRITES Ch55 (a normal chapter run — see notes/state.md for the Ch55 seed). The next weekly audit is Ch49–55 and falls AFTER Ch55 (i.e. the run after next is the audit, not a chapter)._
